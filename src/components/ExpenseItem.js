...
function ExpenseItem(props) {
  ...
  sessionStorage.setItem('hotelId',props.hotelId);

  const navigate =  useNavigate();

  let handleClick = () => {
    let nav_checkout  = "/Checkout?checkin="+props.searchParams.checkInDate+"&checkout="+props.searchParams.checkOutDate+"&price="+props.price+"&noogguests="+props.searchParams.numberOfGuests;
    console.log(nav_checkout);
    navigate(nav_checkout);
  }
  ...
  return (
    <div className="card-container">
      <div class="card-content">
        ...
        <div>
          <p class="card-heading">{props.hotel}</p>
        </div>
        <div class="flexbox">
          <div class="flexbox">
            <img
        ...
        <div>
          ...
          <button type="button" class="btn btn-width" style={{backgroundColor:'#7c3fd8',color:'white'}} onClick={handleClick}>Book @ ${props.price} / Night</button>
        </div>
      </div>
    </div>
  );
